...
function lz1.compress(data,print)
  ...
  for i = 1, 255 do
    ...
    dictionary[string.char(i)] = tohex(1):sub(2,2) .. tohex(i)
    dictionaryi = i
  end
  -- encode
  local pos = 1
  ...
  while pos <= #data do
    ...
    local chunk = ''
    repeat
      chunk = data:sub(pos,pos+lm-1)
      if dictionary[chunk] then
        local olap = tonumber(dictionary[chunk]:sub(1,1),16) + tonumber(dictionary[chunk]:sub(2),16)
    ...
    if (#temp_enc % 2) == 0 then
      ...
      for i = 1, (#temp_enc/2) do
        tmp = tmp .. string.char(tonumber(temp_enc:sub(i*2,(i*2)+1),16))
      end
      encoded = encoded .. tmp
      temp_enc = ''
    end
  ...
  if temp_enc ~= '' then
    ...
    temp_enc = '-' .. temp_enc
    for i = 1, (#temp_enc/2) do
      retval = retval .. string.char(tonumber(temp_enc:sub(i*2,(i*2)+1),16))
    end
...
function lz1.decompress(rawdata,print)
  ...
  local mstring = ''
  for i = 1, 32 do
    mstring = mstring .. tohex(string.byte(rawdata:sub(rawpos,rawpos)))
    rawpos = rawpos + 1
  end
  ...
  while pos < #mstring do
    -- get key size
    local keysize = tonumber(mstring:sub(pos,pos),16)
    if (not keysize) or keysize == 0 then break end
    -- get key
    local key = mstring:sub(pos+1,(pos+1)+keysize-1)
    if (#mstring:sub(pos+1,(pos+1)+keysize-1)) ~= keysize then break end
    -- break key
    local word_len = tonumber(key:sub(1,1),16)
    local pointer = tonumber(key:sub(2),16)
    --
    if ((pos/(#rawdata*2))*100)%5 <= 0.002 then
      print( math.floor((pos/(#rawdata*2))*100) )
    end
    -- decode
    temp = temp .. temp:sub(pointer,pointer+word_len-1)
    pos = pos + keysize + 1
    -- in-situ conversion
